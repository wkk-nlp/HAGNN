...
class WSGATLayer(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, feat_embed_size):
        ...
        self.feat_fc = nn.Linear(feat_embed_size, out_dim, bias=True)
...
class SWGATLayer(nn.Module, ABC):
    def __init__(self, in_dim, out_dim, feat_embed_size):
        super().__init__()
        self.fc = nn.Linear(in_dim, out_dim, bias=True)
...
class SAGATLayer(nn.Module, ABC):
    def __init__(self, in_dim, out_dim, feat_embed_size):
        super().__init__()
        self.fc = nn.Linear(in_dim, out_dim, bias=True)
    ...
    def forward(self, g, h):
        ...
        snode_id = g.filter_nodes(lambda nodes: nodes.data["unit"] == 1)
        saedge_id = g.filter_edges(lambda edges: (edges.src["unit"] == 1) & (edges.dst["unit"] == 2))
        z = self.fc(h)
        g.nodes[snode_id].data['z'] = z
        g.apply_edges(self.edge_attention, edges=saedge_id)
...
class ASGATLayer(nn.Module, ABC):
    def __init__(self, in_dim, out_dim, feat_embed_size):
        super().__init__()
        self.fc = nn.Linear(in_dim, out_dim, bias=True)
